...
text='(sample.java:7)', type=<TextType.ERROR_OCCURS_ROW_IN_CODE: 3>)]
    ...
    for row_idx, line in enumerate(lines, start=1):
        ...
        if error_row:
            ...
            error_occurs_rows.append(HighlightTextInfo(row_idx, TextIndices(error_row.start(), error_row.end()),
                                     res, TextType.ERROR_OCCURS_ROW_IN_CODE))
...
col_idxes=TextIndices(start=27, end=68), text='java.lang.ArrayIndexOutOfBoundsException:', \
type=<TextType.ERROR_MESSAGE: 1>)]
    """
    lines = error.rstrip('\n').splitlines()
